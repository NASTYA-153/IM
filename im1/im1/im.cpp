...
void generateHistogram(vector<double> data, int numBins) {
    ...
    double maxVal = *max_element(data.begin(), data.end());
        ...
        // —оздаем буфер дл€ хранени€ частот
    ...
    for (double value : data) {
        ...
        if (binIndex == numBins) {
            binIndex = numBins; // ”чтем точки, которые равны maxVal
        }
    ...
    // ¬ычисл€ем кол-во частот
    for (int i = 0; i < numBins; i++) {
        totalCount += histogram[i];
    }

    // ¬ыводим гистограмму и кол-во частот
    for (int i = 0; i < numBins; i++) {
    ...
    // ќжидаемое количество точек в каждом отрезке дл€ равномерного распределени€
    int expectedCount = data.size() / numBins;

    // ¬ычисл€ем статистику хи-квадрат
    ...
    //  ритическое значение дл€ 0.05 уровн€ значимости и числа степеней свободы 19
...
int main() {
    ...
    fibDouble.push_back(0.0); // добавл€ем начальное значение вектора fibDouble

    double minVal = 0.0;

    double maxVal = pow(2, 63) - 1; // ћаксимальное значение дл€ делени€ на интервалы
